...
Code is written in TypeScript with two-space indentation and trailing commas omitted. Keep class names in PascalCase (`RigidBody`), exported constants in SCREAMING_SNAKE_CASE, and local variables/functions in camelCase. Prefer explicit return types on public APIs. The codebase relies on vector math helpers; import from `@luz/vectors` rather than relative paths. Run `npx eslint .` (configured in `modules/tests`) before submitting to ensure style compliance.
