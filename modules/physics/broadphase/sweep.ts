...
export function sweepAndPrunePairs<T>(entries: Array<AABBEntry<T>>): Array<[T, T]> {
  ...
  const sorted = entries.slice().sort((a, b) => a.aabb.minX - b.aabb.minX)
...
export function sweepAndPrunePairsAB<A, B>(aEntries: Array<AABBEntry<A>>, bEntries: Array<AABBEntry<B>>): Array<[A, B]> {
  ...
  const aSorted = aEntries.slice().sort((a, b) => a.aabb.minX - b.aabb.minX)
  const bSorted = bEntries.slice().sort((a, b) => a.aabb.minX - b.aabb.minX)
  ...
  for (const ae of aSorted) {
    while (bStart < bSorted.length && bSorted[bStart].aabb.minX <= ae.aabb.maxX) {
